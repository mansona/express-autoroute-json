...
module.exports = function (options) {
	var fieldsToSelect = options.find.select ? options.find.select() : null;
		...
		return function (req, res) {
			...
			.then(function(count){
				//Do the find and pass the results and meta to the process
			 	options.model.find(req.autorouteQuery).skip(req.autorouteOffset).limit(req.autorouteLimit).sort(req.autorouteSort).select(fieldsToSelect).lean(isLean).exec()
	...
	return function (req, res) {
		var query = options.model.find(req.autorouteQuery).sort(req.autorouteSort).select(fieldsToSelect).lean(isLean);
